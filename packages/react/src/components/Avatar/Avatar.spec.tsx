...
describe("Avatar", () => {
  ...
  it("applies the default classes (md size, circle shape, mint color)", () => {
    render(<Avatar initials="OK" />);
    const avatar = screen.getByText("OK").closest(".okkly-avatar");
    expect(avatar).toHaveClass("okkly-component", "okkly-avatar");
    expect(avatar?.className).not.toMatch(/okkly-avatar--(sm|lg)/);
    expect(avatar?.className).not.toMatch(/okkly-avatar--rounded/);
    expect(avatar?.className).not.toMatch(/okkly-avatar--color-(dante|indigo)/);
  });

  it("applies a size modifier only for non-md sizes", () => {
    const { rerender } = render(<Avatar initials="OK" size="sm" />);
    expect(screen.getByText("OK").closest(".okkly-avatar")).toHaveClass("okkly-avatar--sm");
  ...
  it("applies the rounded shape modifier", () => {
    ...
    expect(screen.getByText("OK").closest(".okkly-avatar")).toHaveClass("okkly-avatar--rounded");
  });
  ...
  it("applies a color modifier only for non-mint colors", () => {
    ...
    expect(screen.getByText("AB").closest(".okkly-avatar")?.className).not.toMatch(
      /okkly-avatar--color-(dante|indigo)/,
    );
  });
  ...
  it("renders an image when src is set, and hides the initials", () => {
    ...
    expect(
      screen.getByRole("img", { name: "Oleksii" }).querySelector(".okkly-avatar__image"),
    ).toBeInTheDocument();
  ...
  it("falls back to initials when the image fails to load", () => {
    ...
    const img = document.querySelector(".okkly-avatar__image") as HTMLImageElement;
    fireEvent.error(img);
    expect(screen.getByText("OK")).toBeInTheDocument();
  });
  ...
  it("renders no status dot by default", () => {
    ...
    expect(container.querySelector(".okkly-avatar__status")).not.toBeInTheDocument();
  });

  it("renders an online status dot without a modifier class", () => {
    const { container } = render(<Avatar initials="OK" status="online" />);
    const dot = container.querySelector(".okkly-avatar__status");
    expect(dot).toBeInTheDocument();
    expect(dot).not.toHaveClass("okkly-avatar__status--offline");
  });

  it("renders an offline status dot with its modifier class", () => {
    const { container } = render(<Avatar initials="OK" status="offline" />);
    expect(container.querySelector(".okkly-avatar__status--offline")).toBeInTheDocument();
  });
