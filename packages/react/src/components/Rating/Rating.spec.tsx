...
describe("Rating", () => {
  it("renders the default classes (warning color, medium size)", () => {
    ...
    expect(root).toHaveClass("okkly-component", "okkly-rating", "okkly-rating--read-only");
    expect(root.className).not.toMatch(/okkly-rating--color-/);
    expect(root.className).not.toMatch(/okkly-rating--(small|large)/);
  });
  ...
  it("renders half-filled stars for fractional values", () => {
    ...
    expect(container.querySelectorAll(".okkly-rating__icon--half")).toHaveLength(1);
    expect(container.querySelectorAll(".okkly-rating__icon--full")).toHaveLength(2);
  });

  it("applies size modifier only for non-medium sizes", () => {
    const { rerender, container } = render(<Rating value={4} size="small" readOnly />);
    expect(container.firstChild).toHaveClass("okkly-rating--small");

    rerender(<Rating value={4} size="medium" readOnly />);
    expect((container.firstChild as HTMLElement).className).not.toMatch(
      /okkly-rating--(small|large)/,
    );
  });
  ...
  it("applies color modifier only for non-default colors", () => {
    ...
    expect(container.firstChild).toHaveClass("okkly-rating--color-primary");

    rerender(<Rating value={4} color="warning" readOnly />);
    expect((container.firstChild as HTMLElement).className).not.toMatch(/okkly-rating--color-/);
  });
  ...
  it("updates in uncontrolled mode", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "4 Stars" }));
    expect(
      screen.getByRole("button", { name: "4 Stars" }).querySelector(".okkly-rating__icon--full"),
    ).toBeTruthy();
  });

  it("renders trailing label text", () => {
    render(<Rating value={4.5} label="4.8 · 128 reviews" readOnly />);
    expect(screen.getByText("4.8 · 128 reviews")).toHaveClass("okkly-rating__label");
  });
  ...
  it("disables interaction when disabled", () => {
    ...
    expect(container.firstChild).toHaveClass("okkly-rating--disabled");
    expect(screen.queryByRole("button")).not.toBeInTheDocument();
  ...
  it("forwards ref to the root element", () => {
    ...
    expect(ref.current).toHaveClass("okkly-rating");
  });
});
