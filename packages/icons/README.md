...
```sh
pnpm add @okryshto/icons
```

## Usage

```ts
import { iconArrowUpRight, iconSearch } from "@okryshto/icons";
...
.icon {
  display: inline-flex;
  color: var(--okryshto-color-text-primary);
}
.icon svg {
  width: 1.5rem;
  height: 1.5rem;
...
```ts
import searchUrl from "@okryshto/icons/search.svg";
```
...
```ts
import { ICON_METADATA, groupIconsByCategory, getIconImportName } from "@okryshto/icons/utils";
...
3. Run `pnpm --filter @okryshto/icons generate` and commit the regenerated
   `src/generated.ts`.
